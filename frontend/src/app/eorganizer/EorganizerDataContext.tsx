...
const EorganizerDataContext = createContext<EorganizerDataValue | null>(null);

export function EorganizerDataProvider({ children }: { children: React.ReactNode }) {
  ...
  return <EorganizerDataContext.Provider value={value}>{children}</EorganizerDataContext.Provider>;
}

export function useEorganizerData(): EorganizerDataValue {
  const ctx = useContext(EorganizerDataContext);
  if (!ctx) throw new Error("useEorganizerData must be used within EorganizerDataProvider");
  return ctx;
}
