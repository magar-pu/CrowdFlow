...
import { EorganizerDataProvider, useEorganizerData } from "../EorganizerDataContext";
...
function parseSegments(pathname: string): string[] {
  return pathname.replace(/^\/eorganizer\/?/, '').split('/').filter(Boolean);
}
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  const { adminName, adminRole, toast, handleResetData, handleTriggerLiveScan, events, logs } = useEorganizerData();
  ...
  const setView = (view: AppView) => {
    ...
    const map: Partial<Record<AppView, string>> = {
      dashboard: '/eorganizer',
      events: '/eorganizer/events',
      orders: '/eorganizer/orders',
      attendees: '/eorganizer/attendees',
      finance: '/eorganizer/finance',
      reports: '/eorganizer/reports',
      settings: '/eorganizer/settings',
      'create-event': '/eorganizer/events/create',
    };
    router.push(map[view] ?? '/eorganizer');
  };
...
export default function EorganizerShell({ children }: { children: React.ReactNode }) {
  return (
    <EorganizerDataProvider>
